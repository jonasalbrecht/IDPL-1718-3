...
# sequence_length = avgsequence_length
X_train = sequence.pad_sequences(_X_train_, maxlen=avgsequence_length)
X_test = sequence.pad_sequences(X_test, maxlen=avgsequence_length)
top_words += 1
...
model.add(Embedding(top_words, output_dim =avgsequence_length , input_length=avgsequence_length))
...
rms = optimizers.RMSprop(lr=5e-5)
