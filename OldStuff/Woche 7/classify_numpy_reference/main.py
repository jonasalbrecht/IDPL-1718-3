...
mydata = json.load(open("QAPairs.json"))
i=0
for d in mydata["QsACList"]:
    for x in d["questions"]:
        training_data.append({"class": i, "sentence": x})
    i+=1
    if i==10: break
...
train(X, y, hidden_neurons=20, alpha=1, epochs=10000, dropout=False, dropout_percent=0.2)
...
classify("numpy input : reading lines : .txt : throws : However, assign , fine. take input numpy array?")
